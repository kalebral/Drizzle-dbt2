...
#define FPRINTF(a, b, c) \
	if (mode_string == MODE_SAPDB) { \
		METAPRINTF((a, "\""b"\"", c)); \
	} else if (mode_string == MODE_PGSQL || mode_string == MODE_MYSQL || mode_string == MODE_DRIZZLE) { \
		METAPRINTF((a, b, c)); \
	}
#define FPRINTF2(a, b) \
	if (mode_string == MODE_SAPDB) { \
...
void escape_me(char *str)
{
	...
	if (mode_string == MODE_PGSQL || mode_string == MODE_MYSQL || mode_string == MODE_DRIZZLE) {
		strcpy(buffer, str);
		i = strlen(buffer);
		for (k = 0; k <= i; k++) {
			if (buffer[k] == '\\') {
	...
	for (i = 0; i < warehouses; i++) {
		for (j = 0; j < DISTRICT_CARDINALITY; j++) {
			...
			for (k = 0; k < orders; k++) {
				...
				/* Drizzle doesn't handle null value for
				   when reading from a file for now
				*/
